...
process OVERLAPPINGANNOTATIONS_overlapping {
    ...
    '''
        ...
        echo $s2 | xargs paste  | awk -v s1="$s1" 'BEGIN{OFS="\\t"; print s1} {s=""; for(i=3; i<=(NF-3); i+=3){s=(s)($i)(" ")} print (s)($NF)}' > $base.coord.over.allannots.txt
...
/*
//And finally a file with only 0 and 1 with as many columns as above annot, preserving the snps' order
process OVERLAPPINGANNOTATIONS_merge {
    cpus 1
    memory 8.GB

    publishDir params.outputDir_merge, mode: 'copy'
    ...
    output:
        path '*.coord.over.allannots.txt'
    

    shell:
   '''
        base=`basename !{tsvannfiles}`
        s1=`cat !{annotations} | awk '{s=(s)($1)(" ")} END{print s}'`
        s2=`cat !{annotations} | while read annid annfile; do echo $base.coord.over.$annid ; done | awk '{s=(s)($1)(" ")} END{print s}'`
        echo $s2 | xargs paste  | awk -v s1="$s1" 'BEGIN{OFS="\\t"; print s1} {s=""; for(i=3; i<=(NF-3); i+=3){s=(s)($i)(" ")} print (s)($NF)}' > $base.coord.over.allannots.txt
        check_simple.sh $base.coord.over.allannots.txt
   '''
